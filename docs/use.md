...
If you are using the [Windows executables]({{ site.baseurl }}//install/#windows-executables), substitute `mailbagit.exe` for the `mailbagit` command. e.g `mailbagit.exe path/to/messages -i msg -d mbox pdf-chrome`
...
## Using the `mailbagit-gui` graphical user interface
